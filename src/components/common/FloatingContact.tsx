...
const hotelsData: Record<string, { name: string; address: string }> = {
    'hestia-stay-kasana': {
...
export default function FloatingContact() {
    ...
    const hotelName = hotel?.name || 'Hestia Hotels';
    const hotelAddress = hotel?.address || '';
    
    const message = `Hello ${hotelName}!%0A%0AI would like to inquire about room availability and booking.%0A%0AHotel: ${hotelName}${hotelAddress ? `%0AAddress: ${hotelAddress}` : ''}%0A%0AThank you!`;

    return (
        <div className="fixed bottom-6 right-6 flex flex-col gap-3 z-50">
            {/* WhatsApp Button */}
            <a
                href={`https://wa.me/919990555740?text=${message}`}
                target="_blank"
            ...
            <a
                ...
                className="group bg-blue-500 hover:bg-blue-600 text-white p-4 rounded-full shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-110"
                aria-label="Call us"
            >
